...
module.exports = (thorin, opt) => {
  const logger = thorin.logger(opt.logger),
    store = Symbol("store"),
    defaultChannel = opt.channel || 'thorin.queue';
  const VIRTUAL_QUEUE = {};
  ...
  class Queue {
    ...
    run(done) {
      ...
      if (this.connected && !this.virtual) {
        drainVirtual();
      }
      storeObj.on('connect', () => {
        this.connected = true;
  ...
  function drainVirtual() {
    Object.keys(VIRTUAL_QUEUE).forEach((channel) => {
      let payloads = VIRTUAL_QUEUE[channel];
      delete VIRTUAL_QUEUE[channel];
      for(let i=0, len = payloads.length; i < len; i++) {
        queueObj.enqueue(JSON.parse(payloads[i]), channel);
      }
    });
  }
  ...
  function doDequeue(count, channel, fn) {
    ...
    // Otherwise, we're going to use LPOP to spare commands
    this[store].exec('LPOP', channel, (err, item) => {
      if (err) return fn(err);
      try {
        item = JSON.parse(item);
      } catch (e) {
        logger.warn(`Could not parse previously enqueued item ${item} in channel ${channel}`);
      }
      fn(null, item);
    });
  }
